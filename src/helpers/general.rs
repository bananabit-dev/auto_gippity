...
pub fn extend_ai_function(ai_func: fn(&str) -> &'static str, func_input: &str) -> Message {
    ...
    let msg: String = format!(
        r#"You are a function printer. Output ONLY valid JSON. No commentary.
Here is the function:
{}

Input: {}

Respond ONLY with a JSON object matching the function's return type."#,
...
pub async fn ai_task_request_decoded<T: DeserializeOwned>(
    ...
    dbg!("llm raw output for decoding", &llm_response);

    let json_str = extract_json(&llm_response)
        .expect("No JSON object/array found in LLM response");

    let decoded_response: T = serde_json::from_str(json_str)
...
mod tests {
    ...
    use crate::{
        ...
        helpers::command_line::ai_task_request,
    };
